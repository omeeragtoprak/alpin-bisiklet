...
const ProductModelViewer = dynamic(
	() =>
		import("@/components/3d/product-model-viewer").then(
			(m) => m.ProductModelViewer,
		),
	{ ssr: false, loading: () => <div className="aspect-square bg-muted/40 rounded-xl animate-pulse" /> },
);
...
export function ProductDetailClient({ product }: { product: ProductDetailData }) {
	...
	// Merge images + videos into a single gallery list
	const galleryItems: GalleryItem[] = [
		...product.images.map((img) => ({ kind: "image" as const, ...img })),
		...product.videos.map((vid) => ({ kind: "video" as const, ...vid })),
	];

	const currentItem = galleryItems[selectedIndex] ?? null;
	const isBicycle = product.category?.type === "BICYCLE";
	...
	const prevItem = () => setSelectedIndex((i) => Math.max(0, i - 1));
	const nextItem = () =>
		setSelectedIndex((i) => Math.min(galleryItems.length - 1, i + 1));
				...
				<div className="space-y-4">
					...
					>
						{currentItem?.kind === "video" ? (
							/* GIF veya video — controls yok */
							currentItem.url.toLowerCase().endsWith(".gif") ? (
								<img
									key={currentItem.url}
									src={currentItem.url}
					...
					<Tabs defaultValue="description">
						...
						{product.model3dUrl && (
							<TabsContent value="model3d" className="mt-6">
								<div className="space-y-3">
									<p className="text-sm text-muted-foreground">
										Modeli döndürmek için sürükleyin. Yaklaştırmak/uzaklaştırmak için kaydırın.
									</p>
									<ProductModelViewer
